...
function simple_social_bookmarks($url,$twitter_text,$style,$add_paras) {
    ...
    $twitter_text=str_replace('%title%',get_the_title($post->ID),$twitter_text);
    $echoout="";
    ...
    if (($option=="on")or(($default=="on")&&($option!="off"))) {
        $echoout=$echoout."<a href=\"http://twitter.com/home?status=".urlencode($twitter_text)."\" target=\"_blank\"><img src=\"".$plugin_url."twitter.png\" alt=\"Share with Twitter\" title=\"Share with Twitter\" style=\"".$style."\"></a>\n";
    }
    return $echoout;
}
