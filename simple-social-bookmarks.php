...
function simple_social_bookmarks($url,$twitter_text,$style,$add_paras) {
    ...
    if (function_exists('simple_url_shortener')) {
        $tinyurl=simple_url_shortener($url,"service=".$service.$cache);
    } else {
        $tinyurl=$url;
    }
    ...
    if ($option=="on") {
        $echoout=$echoout."<a href=\"mailto:?subject=".$email_title."&amp;body=".$email_text."\" target=\"".$target."\"".$nofollow."><img src=\"".$icon_folder."email.png\" alt=\"Email\" title=\"Email\" style=\"".$style."\"/></a>\n";
    }
    ...
    if ($option=="on") {
        $echoout=$echoout."<a href=\"http://www.google.com/reader/link?url=".$url."&amp;title=".$title."\" target=\"".$target."\"".$nofollow."><img src=\"".$icon_folder."gbuzz.png\" alt=\"Add to Google Buzz\" title=\"Add to Google Buzz\" style=\"".$style."\"/></a>\n";
    }  
    ...
    if ($option=="on") {
        $echoout=$echoout."<a href=\"http://www.technorati.com/faves?add=".$url."\" target=\"".$target."\"".$nofollow."><img src=\"".$icon_folder."technorati.png\" alt=\"Add as a Technorati Favorite\" title=\"Add as a Technorati Favorite\" style=\"".$style."\"/></a>\n";
    }    
    ...
    if ($option=="on") {
        $echoout=$echoout."<a href=\"http://bookmarks.yahoo.com/myresults/bookmarklet?u=".$url."&amp;t=".$title."\" target=\"".$target."\"".$nofollow."><img src=\"".$icon_folder."yahoo.png\" alt=\"Save to Yahoo! Bookmarks\" title=\"Save to Yahoo! Bookmarks\" style=\"".$style."\"/></a>\n";
    }
    $option=strtolower(social_bookmarks_parameters($add_paras,"yahoobuzz"));
    if ($option=="on") {
        $echoout=$echoout."<a href=\"http://buzz.yahoo.com/buzz?targetUrl=".$url."&amp;headline=".$title."\" target=\"".$target."\"".$nofollow."><img src=\"".$icon_folder."ybuzz.png\" alt=\"Buzz up!\" title=\"Buzz up!\" style=\"".$style."\"/></a>\n";
    }    
    return $echoout; 
}
